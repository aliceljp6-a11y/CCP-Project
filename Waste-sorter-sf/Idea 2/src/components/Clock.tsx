...
export function Clock({ minutesFromMidnight, running }: ClockProps) {
  ...
  return (
    <div
      className="flex flex-col items-end gap-1 rounded-xl px-3 py-2 shadow-sm backdrop-blur-sm xl:px-4 xl:py-3 2xl:px-5 2xl:py-4"
      style={{
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="font-display text-[2rem] font-bold leading-none tabular-nums xl:text-[2.35rem] 2xl:text-[2.7rem]" style={{ color: '#1A1A1A' }}>
          {hour12}:{minute === 0 ? '00' : String(minute).padStart(2, '0')} {ampm}
        </span>
      </div>
      <span className="text-[10px] font-semibold uppercase tracking-wider xl:text-xs 2xl:text-sm" style={{ color: '#555555' }}>
        {label} · 7:30 AM – 4:00 PM
      </span>
      {running && (
        <span className="text-[10px] xl:text-xs 2xl:text-sm" style={{ color: '#555555' }}>1 hour = {REAL_SECONDS_PER_HOUR}s</span>
      )}
    </div>
  )
}
